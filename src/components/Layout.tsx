...
import { LayoutDashboard, List, Columns, PlusCircle, BarChart3, Settings, FileText, ChevronLeft, ChevronRight, Bell, Search, Zap } from 'lucide-react';
      ...
      <aside style={{ width: collapsed ? 64 : 220, background: '#fff', boxShadow: '2px 0 16px rgba(0,0,0,0.05)', flexShrink: 0, display: 'flex', flexDirection: 'column', transition: 'width 0.25s' }}>
          ...
          <div style={{ padding: '10px 12px', borderBottom: '1px solid #f1f5f9' }}>
            <div style={{ display: 'flex', background: '#f1f5f9', borderRadius: 12, overflow: 'hidden', padding: 3, gap: 3 }}>
              {(['VIRTAVO', 'ShowMo'] as const).map(b => {
                ...
                const activeBg = b === 'VIRTAVO' ? '#4FA7A0' : '#c8dc00';
                const activeText = b === 'VIRTAVO' ? '#fff' : '#3d5200';
                return (
                  <button
                    key={b}
                  ...
                  >
                    <span style={{ width: 6, height: 6, borderRadius: 99, background: isActive ? (b === 'VIRTAVO' ? '#fff' : '#3d5200') : '#94a3b8', display: 'inline-block', flexShrink: 0 }} />
                    {b}
                  </button>
                );
              })}
            ...
            <div style={{ marginTop: 6, fontSize: 10, color: '#94a3b8', textAlign: 'center' }}>
              当前：<span style={{ color: activeBrand === 'VIRTAVO' ? '#4FA7A0' : '#8aad00', fontWeight: 700 }}>{activeBrand}</span>
            </div>
          </div>
        )}
        ...
        <nav style={{ flex: 1, padding: '10px 8px', overflowY: 'auto' }}>
          {NAV.map(({ path, icon: Icon, label }) => {
            ...
            const activeBg = activeBrand === 'VIRTAVO'
              ? 'linear-gradient(135deg,#4FA7A0,#3a8f89)'
              : 'linear-gradient(135deg,#c8dc00,#a8bb00)';
            const activeText = activeBrand === 'ShowMo' ? '#2a3800' : '#fff';
            return (
              <NavLink key={path} to={path} title={collapsed ? label : undefined} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', marginBottom: 2, borderRadius: 12, textDecoration: 'none', background: active ? activeBg : 'transparent', color: active ? activeText : '#64748b', fontWeight: active ? 600 : 400, fontSize: 13, transition: 'all 0.15s' }}>
